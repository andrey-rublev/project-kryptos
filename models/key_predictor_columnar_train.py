...
# defaults
DEFAULT_DATA_PATH = r"C:\Users\nikhi\Downloads\Kryptos\data\output\columnar.csv"
DEFAULT_MODEL_PATH = r"C:\Users\nikhi\Downloads\Kryptos\models\key_predictor_columnar.pt"

ALL_CHARS = string.ascii_lowercase + " .,!?'"
CHAR2IDX = {c: i for i, c in enumerate(ALL_CHARS)}
VOCAB_SIZE = len(ALL_CHARS)
NUM_CLASSES = 7  # choices 3..9


# make sure encode package is importable
import sys, os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
from encode.columnar import columnar_transposition_encrypt
...
class ColumnarDataset(torch.utils.data.Dataset):
    def __init__(self, csv_path, max_len, augment=0):
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                key = row["key"]
                ncols = len(key)
                self.samples.append((encode_text(ciphertext, max_len), ncols))
                for _ in range(augment):
                    new_n = random.randint(3, 9)
                    # generate a random permutation of length new_n
                    perm = list(range(1, new_n + 1))
                    random.shuffle(perm)
                    new_cipher = columnar_transposition_encrypt(plaintext, perm)
                    self.samples.append((encode_text(new_cipher, max_len), new_n))
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(x, dtype=torch.long), torch.tensor(y-3, dtype=torch.long)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Columnar key predictor")
    ...
    full_dataset = ColumnarDataset(args.data_path, args.max_len, augment=args.augment)
    ...
    model = ColumnarKeyPredictor(VOCAB_SIZE,
